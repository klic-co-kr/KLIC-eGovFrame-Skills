...
```bash
# 스킬 디렉터리로 클론
git clone https://github.com/<owner>/klic-maintenance-skills.git
# 각 스킬을 개인 스킬 디렉터리에 복사
cp -r klic-maintenance-skills/skills/klic-* ~/.claude/skills/
```

또는 심볼릭 링크로 연결(업데이트 편의):

```bash
for d in klic-maintenance-skills/skills/klic-*; do
...
```bash
cp -r klic-maintenance-skills/skills/klic-* ~/.agents/skills/
```
