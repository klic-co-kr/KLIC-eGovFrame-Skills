...
| **klic-effort-router** | 작업 티어(S/M/L/XL) 판정 → 단계별(계획/검토/구현/리뷰) 모델·에포트 배정, 정의된 서브에이전트 파일로만 라우팅. M티어 이상은 §5 상태·인계 계약(증거번들 인계·state.json 진행·조건부 롤백) 적용 | 작업 착수 전 티어 판정, 서브에이전트 위임, 단계 간 인계·진행 상태 운반, 보안 감사 준비 |
